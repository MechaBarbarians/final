...
let incrementer = 1;
let leftPos = 0;
let isAnimating = false; // Flag to track if animation is already running

function testRequestAnimationFrame() {
    if (isAnimating) return; // Prevent starting multiple animation loops

    let startButton = document.getElementById("startAnimation");
    startButton.disabled = true;
    let stopButton = document.getElementById("stopAnimation");
    stopButton.disabled = false;
    incrementer = 1; // Ensure consistent speed
...
function stopAnimationFrame() {
    ...
    incrementer = 0; // Stop the animation
    isAnimating = false; // Reset the flag to allow restarting the animation
}
...
function animationLoop() {
    ...
    if (incrementer !== 0) {
        ...
        isAnimating = false; // Ensure the flag is reset when animation stops
    }
}
